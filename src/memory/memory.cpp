...
uint8_t *Memory::find_pattern(const char *module_name, const char *sig, unsigned int byte_offset) noexcept {
  const auto module = this->get_module_base(module_name);
  ...
  for (auto i{0ul}; i < size_of_image - size; ++i) {
    ...
    if (found)
      return &scanBytes[i] + byte_offset;
  }
